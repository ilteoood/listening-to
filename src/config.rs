...
mod tests {
    ...
    fn test_from_env_uses_default_values() {
        ...
        assert_eq!(config.spotify_token_cache_path, std::path::PathBuf::from(".spotify_token_cache.json"));
    ...
    fn test_from_env_with_custom_optional_vars() {
        ...
        assert_eq!(config.spotify_token_cache_path, std::path::PathBuf::from("/custom/path/token.json"));
    ...
    fn test_from_env_missing_spotify_client_id() {
        ...
        assert!(result.unwrap_err().to_string().contains("SPOTIFY_CLIENT_ID not set"));
    }
    ...
    fn test_from_env_missing_spotify_client_secret() {
        ...
        let result = Config::from_env();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("SPOTIFY_CLIENT_SECRET not set"));
    }
    ...
    fn test_from_env_missing_slack_token() {
        ...
        let result = Config::from_env();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("SLACK_TOKEN not set"));
    }
    ...
    fn test_from_env_missing_slack_cookie() {
        ...
        let result = Config::from_env();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("SLACK_COOKIE not set"));
    }
